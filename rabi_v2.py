...
class PhaseLockedLoop(QMainWindow, Ui_PhaseLockedLoop):
    def __init__(self):
        ...
        self.txtIP.setText('rp-f09ded.local')
        ...
        self.txtCountDuration.setText('10000')
    ...
    def apply(self):
        try:
            ...
            self.plot_odmr()
            if self.comboBoxMWdevice.currentIndex() == 0:
                centerFreq = ('fx' + str(self.CenterFreq)).encode()
                power = ('W' + str(self.RFPower)).encode()
                powerSweepStart = ('[' + str(self.RFPower)).encode()
                powerSweepStop = (']' + str(self.RFPower)).encode()
                startFreq = ('l' + self.txtStartFreq.text()).encode()
    ...
    def range_changed(self):
        try:
            ...
            self.Range = np.arange(float(self.Range[0]), float(self.Range[1]) + float(self.Range[2]),
                                   float(self.Range[2]))
            self.Step = float(self.Range[1]) - float(self.Range[0])
        except Exception:
            traceback.print_exc()
