...
try:
    ...
    print("The pyvisa module is not installed.")

"""
---------------------------------------------------------
This file reads bin/csv file of position acquired from the Red Pitaya and generates a PSD of position in V^2/hz and a V/s plot
"""
...
def run_code():
    ...
    if is_new_measurement_checked():
        ...
        if import_successful:
            ...
            trap_frequency, trap_amplitude, trap_offset = "Not Available", "Not Available", "Not Available"
    ...
    try:
        ...
        figure_headline_text = f'{trap_frequency}, {trap_amplitude}, {trap_offset}'

    except IndexError:
        print('No Data on trap parameters from old measurements')
        print(IndexError)
        figure_headline_text = f'No Trap Parameters'


    # Clear and add subplots to figure
    fig.clear()
    fig.suptitle(figure_headline_text)
...
checkbox_new_measurement = tk.Checkbutton(root, text='New measurement (Always Tag when working with .bin files)', variable=new_measurement_var)
...
fig.suptitle('Select a File', fontsize=50)
